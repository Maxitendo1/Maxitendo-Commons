...
import com.maxitendo.commons.extensions.sha1
import com.maxitendo.commons.extensions.sha256
...
class PropertiesDialog : BasePropertiesDialog {
    ...
    private fun addProperties(path: String) {
        ...
        ensureBackgroundThread {
            ...
            this.mActivity.runOnUiThread {
                ...
                if (fileDirItem.isDirectory) {
                    (mDialogView.propertiesHolder.findViewById<RelativeLayout>(R.id.properties_file_count).findViewById<MyTextView>(R.id.property_value)).text =
                        fileCount.toString()
                    (mDialogView.propertiesHolder.findViewById<RelativeLayout>(R.id.properties_direct_children_count)
        ...
        if (fileDirItem.isDirectory) {
            ...
            if (mActivity.baseConfig.appId.removeSuffix(".debug") == "com.maxitendo.filemanager") {
                ...
                calculateAndDisplayHash(
                    ...
                    hashInputStream = { inputStream -> inputStream.getDigest(SHA1) },
                    hashFile = { file -> file.getDigest(SHA1) }
                )
                ...
                calculateAndDisplayHash(
                    ...
                    hashInputStream = { inputStream -> inputStream.getDigest(SHA256) },
                    hashFile = { file -> file.getDigest(SHA256) }
                )
            }
        }
    }
    ...
    private fun updateLastModified(activity: Activity, view: View, timestamp: Long) {
        activity.runOnUiThread {
            (view.findViewById<RelativeLayout>(R.id.properties_last_modified).findViewById<MyTextView>(R.id.property_value)).text = timestamp.formatDate(activity)
        }
    }
